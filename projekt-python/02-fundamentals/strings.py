...
#Řešení
datum = "12. 10. 2020"
databazove_datum = datum.replace(".", "").replace(" ", "-")
databazove_datum.rstrip()
print(databazove_datum)

databazove_datum1 = f'{datum[-4:]}-{datum[4:6]}-{datum[:2]}'
print(databazove_datum1)
def identifikatory(text, camel=False):
    slova = text.split()
    if camel:
        identifikator = slova[0].lower() + ''.join(word.capitalize() for word in slova[1:])
    else:
        identifikator = '_'.join(word.lower() for word in slova)
    return identifikator
text = input("Zadej souslovi: ")
python_id = identifikatory(text, camel=False)
camel_id = identifikatory(text, camel=True)
print(f"{python_id}")
print(f"{camel_id}")
...
"""vstup= input("Zadej souslovi: ")
def souslovi():
    print(vstup.lower().replace(" ", "_").replace('á', 'a').replace('č', 'c').replace('ě', 'e').replace('é', 'e').replace('í', 'i').replace('ň', 'n').replace('ó', 'o').replace('ř', 'r').replace('š', 's').replace('ť', 't').replace('ú', 'u').replace('ů', 'u').replace('ý', 'y').replace('ž', 'z'))

def camel_case(text):
    slova = text.split()
    camel_case_text = slova[0].lower() + ''.join(word.capitalize() for word in slova[1:]).replace('á', 'a').replace('č', 'c').replace('ě', 'e').replace('é', 'e').replace('í', 'i').replace('ň', 'n').replace('ó', 'o').replace('ř', 'r').replace('š', 's').replace('ť', 't').replace('ú', 'u').replace('ů', 'u').replace('ý', 'y').replace('ž', 'z')
    return camel_case_text

vystup = camel_case(vstup)
souslovi()
print(vystup)
"""
...
import random
def hesla():
    vstup = int(input("Zadej počet lidí, pro kolik bude vytvořeno heslo: "))
    hesla_create=[
        "".join(random.choice(string.ascii_uppercase) for i in range(3))+
        "".join(random.choice(string.ascii_lowercase) for j in range(3))+
        random.choice("-/+*") +
        "".join(random.choice(string.digits) for k in range(3))
        for o in range(vstup)
]
    print(hesla_create)
hesla()
