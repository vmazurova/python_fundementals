...
def aktualnidatum():
    dateaktualni = datetime.datetime.now()
    print("Aktuální datum a čas:", dateaktualni)

def velikonoceVpetLetech():
    actualyear = datetime.datetime.now().year
    for rok in range(actualyear, actualyear + 5):
        datum_velikonoc = easter(rok)
        print(f"Velikonoční neděle v roce {rok}: {datum_velikonoc}")

def stedryDenNedele():
    rok = datetime.datetime.now().year
    for rok in range(rok, rok + 100):
        stedryden = datetime.date(rok, 12, 24)
        if stedryden.weekday() == 6:
            print(f"Nejbližší rok, kdy bude Štědrý den v neděli: {rok}")
            return

aktualnidatum()
velikonoceVpetLetech()
stedryDenNedele()
